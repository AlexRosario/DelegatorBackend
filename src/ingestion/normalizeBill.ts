type CombinedBill = {
	bill: any;
	summaries?: any[];
	subjects?: { legislativeSubjects?: any[]; policyArea?: any };
	textVersions?: any[];
	actions?: any[];
};
 ...
 */
function deriveStage(actions: any[] = []): string | null {
	if (!actions.length) return null;
	const texts = actions.map((a) => String(a.text ?? '').toLowerCase());
	const has = (re: RegExp) => texts.some((t) => re.test(t));

	if (has(/became (public|private) law|signed by president/)) return 'Became Law';
	if (has(/vetoed/)) return 'Vetoed';
	if (has(/presented to president|to president/)) return 'To President';
	...
	if (passedHouse && passedSenate) return 'Passed Both Chambers';
	if (passedSenate) return 'Passed Senate';
	if (passedHouse) return 'Passed House';

	if (has(/failed|rejected/)) return 'Failed';
	if (has(/reported by|ordered to be reported|committee/)) return 'In Committee';
	if (has(/introduced/)) return 'Introduced';
	return 'In Committee';
}

/** Most recent summary by action date (summaries arrive oldest-first). */
function pickLatestSummary(summaries: any[] = []) {
...
function pickTextVersionUrl(textVersions: any[] = []): string | null {
	const latest = [...textVersions].sort((a, b) => String(b.date).localeCompare(String(a.date)))[0];
	if (!latest?.formats?.length) return null;
	const formatted = latest.formats.find((f: any) => f.type === 'Formatted Text');
	return (formatted ?? latest.formats[0]).url ?? null;
}

export function normalizeBill({ bill, summaries, subjects, textVersions, actions }: CombinedBill) {
	const sponsor = bill.sponsors?.[0];
	const latestSummary = pickLatestSummary(summaries);
	const subjectNames = (subjects?.legislativeSubjects ?? []).map((s: any) => s.name).filter(Boolean);
