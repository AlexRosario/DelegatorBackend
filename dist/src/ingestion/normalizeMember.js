...
function normalizeChamber(chamber) {
    if (!chamber)
        return null;
    if (/senate/i.test(chamber))
        return 'Senate';
    if (/house/i.test(chamber))
        return 'House';
    return chamber;
}
/** Canonical single-letter party code (D/R/I) — stable regardless of source wording. */
function toPartyCode(raw) {
    if (!raw)
        return '';
    const v = raw.trim().toLowerCase();
    if (v === 'd' || v.startsWith('democrat'))
        return 'D'; // Democrat / Democratic
    if (v === 'r' || v.startsWith('republican'))
        return 'R';
    if (v === 'i' || v.startsWith('independent'))
        return 'I';
    return raw.trim().charAt(0).toUpperCase();
}
/** Current party from partyHistory (prefer abbreviation; fallback to flat partyName). */
function latestParty(member) {
...
function composeOfficeAddress(addr) {
    if (!addr)
        return null;
